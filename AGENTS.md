# AGENTS.md — TaCZ Tweaks NeoForge 26.1.2 维护规则

供 AI 编码助手和人类协作者使用。当前目标是 Minecraft 26.1.2 / NeoForge 26.1.2.97 / Java 25 /
未混淆 / 依赖 `q14433686-arch/TaCZ_Renovated` 的 `1.1.8+neoforge.26.1.2.R1`（modId 仍是 `tacz`）。

移植总览与执行记录：`docs/records/NEOFORGE_26_1_2_PORT_PLAN.md`、
`docs/records/NEOFORGE_26_1_2_PORT_RECORD.md`。姊妹分支：`26.1.2`（Fabric，游戏语义权威）、
`1.21.11-neoforge`（同一套代码的另一条 NeoForge 线，可作范式参考）。

## 1. 事实来源顺序

1. 当前源码调用链与 26.1.2 class descriptor；
2. `libs/tacz-1.1.8+neoforge.26.1.2.R1.jar`（以及 NeoForge/Minecraft 26.1.2 jar）；
3. 目标仓库 `q14433686-arch/TaCZ_Renovated` 的 `26.1.2` 当前源码/文档；
4. 本仓库 `26.1.2` Fabric 分支（游戏语义），只抄语义不抄加载器表面；
...
```bash
python3 scripts/audit_port.py --strict --minecraft-jar /path/to/minecraft-26.1.2.jar
```
...
TaCZ 版本门禁常量在 `TaczVersionSupport`（family `neoforge.26.1.2`，最低 revision R1）。

## 4. 不得夸大

- “实现”“静态验证”“编译通过”“游戏内实测”必须分开表述；
- 跳过、禁用、默认关闭不能写成修复；
- 可选模组兼容必须核对真实 NeoForge 26.1.2 发行物与源码/API，不能凭旧文档说不存在，
...
## 6. NeoForge 26.1.2 常用写法

- 入口：`@Mod` 构造器 `(IEventBus modEventBus, ModContainer container)`；客户端由
  `FMLLoader.getCurrent().getDist().isClient()` 门控；
