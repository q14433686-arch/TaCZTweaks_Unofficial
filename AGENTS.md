# AGENTS.md — TaCZ Tweaks Fabric 26.1.2 维护规则

供 AI 编码助手和人类协作者使用。当前目标是 Minecraft 26.1.2 / Java 25 / 未混淆
`TaCZ_Refabricated_Unofficial` R2。

## 1. 事实来源顺序

1. 当前源码调用链与 26.1.2 class descriptor；
2. `libs/TACZ-Refabricated-26.1.2-1.1.8+fabric.26.1.2.R2.jar`；
3. 目标仓库 `q14433686-arch/TaCZ_Refabricated_Unofficial` 的 `26.1.2` 当前源码/文档；
...
```bash
python3 scripts/audit_port.py --strict --minecraft-jar /path/to/minecraft-merged-26.1.2.jar
```
...
- 跳过、禁用、默认关闭不能写成修复；
- 可选模组兼容必须核对真实 Fabric 26.1.2 发行物与源码/API，不能凭旧文档说不存在；
