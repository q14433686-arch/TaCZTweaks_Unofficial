# 移植笔记 — TaCZ Tweaks → Fabric 26.1.2

记录从 Forge 1.20.1 原版（MUKSC/TaCZTweaks v2.14.2）移植到
`TaCZ_Refabricated_Unofficial`（26.1.2 主分支）的要点，供后续维护者参考。
...
- 26.1.2 分支：**未混淆**（直接用 Mojang 类名，无 mappings），Java 25，Fabric Loom 1.17。
...
## 2. Minecraft API 迁移（1.20.1 → 26.1.2，影响所有打 net.minecraft.* 的 mixin）
...
| Forge 1.20.1（原版 mixin 目标） | 26.1.2 Refabricated（新目标） |
|---|---|
...
### 6.3 因 26.1.2 已不存在的 vanilla 方法而跳过的

- `LocalPlayer.canStartSprinting()` 在 26.1.2 已不存在 → 跳过 vanilla `LocalPlayerMixin`；
...
**无需移植**。目标端 TaCZ 的 `ItemInHandLayerMixin` 已按 26.1.2 的 extract→submit 管线重写，
`submitTail` 内已调用 `HumanoidOffhandRender.renderGun`（并修复了左利手与 `isSelf` 问题）。
`thirdPersonGunRenderingFix` 配置项保留但无对应 mixin。

### 7.5 无尽弹药

- `EndlessAmmoStatusEffect`（Kotlin）：26.1.2 的 `MobEffect(category, color)`；
...
- `LivingEntityAmmoCheckMixin`：`consumesAmmoOrNot` HEAD cancel，
  `player.hasEffect(Holder<MobEffect>)`（26.1.2 签名）。
...
| Mixin | 功能 | 移植要点 |
|---|---|---|
| `tweaks.EnderManMixin` | 末影人躲避子弹 | `hurt` → 26.1.2 的 `hurtServer(ServerLevel, DamageSource, float)`；第一个 `DamageSource.is(TagKey)` 即 `IS_PROJECTILE` 检查（已核字节码） |
| `tweaks.RefitKeyMixin` | 冒险模式禁用改装 | `onRefitPress()` → `onRefitPress(InputEvent.Key)`；`getPlayerMode()/GameType.ADVENTURE` 26.1.2 仍存在 |
...
### 26.1.2 与 26.2 的实体常量差异

26.1.2 merged jar 中存在 `net.minecraft.world.entity.EntityType`，不存在 26.2 的复数
`EntityTypes`；末影人常量仍为 `EntityType.ENDERMAN`。本分支已按目标 jar 恢复单数类，
并用全局 grep 确认没有残留 `EntityTypes` 引用。
...
5. **动态俯仰角**：26.1.2 里 `Level.clip` 移到了 `BlockGetter.clip(ClipContext)`（default 方法，
...
1. 注入点正确：26.1.2 `MouseHandler.turnPlayer(double)` 内 `LocalPlayer.turn(DD)V` 仅一处（offset 292）；
...
3. 姿势判断正确：TaCZ 26.1.2 用 `ForcePoseInjection` 把匍匐姿势强制为 `Pose.SWIMMING`，且
...
| 方块/实体/物品谓词 | `BlockPredicate/EntityPredicate/ItemPredicate` | **26.1.2 已不存在**（predicate 系统 codec 化），需重新设计 |
...
| 假玩家 | Forge `FakePlayer` | 当时误判需砍；Fabric API 26.1.2 实际提供 `net.fabricmc.fabric.api.entity.FakePlayer`，本移植最终选择可测试的纯数学路径 |
| TaCZ 内部 | `EntityKineticBulletAccessor(onHitEntity)`、`BlockRayTrace.rayTraceBlocks` 的 `lambda$rayTraceBlocks$1/2`、`onBulletTick`、`BulletHoleOption`、`TacHitResult` | 需逐一对 26.1.2 字节码考古（lambda 名几乎肯定变了） |
...
- **第 2 轮：行为层**。`BlockRayTracer`/`BlockBreakingManager` + `features.*` mixin + `accessor`，
  考古 26.1.2 的 `EntityKineticBullet` 碰撞流程与 `BlockRayTrace`。
...
原版 40+ 文件的 data 层已完成移植并编译通过。**DFU 10（随 MC 26.1.2）有一批破坏性 API 变更**，
是移植的主要坑：

| 原版（DFU 6 / 1.20.1） | 26.1.2（DFU 10.0.21） | 处理 |
|---|---|---|
...
- `BlockTarget.TIER`、`EntityTarget.PREDICATE`、`Target.PREDICATE/BURST_INDEX/PELLET_INDEX`、
  ...
  `data/old/*` —— 依赖 Forge（TierSortingRegistry、ShieldBlockEvent）或 26.1.2 已删除的
...
- `data/.../tags/blocks/` → `data/.../tags/block/`（26.1.2 目录单数化）
- `#forge:glass` → `#c:glass_blocks`、`#forge:glass_panes` → `#c:glass_panes`（Fabric convention tags）

## 7.15 数据驱动系统 行为层 移植记录（2026-08-17）

行为层已完成并编译通过。核心差异：**26.1.2 的 `EntityKineticBullet#onBulletTick` 已内联整个碰撞流程**
（rayTraceBlocks → findEntitiesOnPath → onHitEntity×N → onHitBlock），原版依赖的
"在 `BlockRayTrace.lambda$rayTraceBlocks$1/2` 拦截"架构改成了对 26.1.2 具名方法的注入：
...
- **FakePlayer + DestroySpeedModifierHolder + BlockBehaviourMixin**：此处“Fabric 无 FakePlayer”是早期误判；
  Fabric API 26.1.2 实际提供 FakePlayer。本移植选择独立 `SafeMath.blockBreakingDelta`，直接计算
...
- **entity pierce 规则**：26.1.2 原 onBulletTick 已有自己的 pierce 循环（枪械 pierce 值），
...
本轮不再按 1.20 类名判断能力，而是核对 26.1.2 class 调用链和实际 Fabric 发行物：
...
- predicate 移到 `net.minecraft.advancements.criterion`，并未删除；MinMaxBounds 同样只是换包；
- tier 语义由 `ToolMaterial.incorrectBlocksForDrops` 承担；
- shield 走 26.1.2 `BlocksAttacks` component，不再寻找旧 `hurtCurrentlyUsedShield`；
- `Level#getGameTime()` 实际存在，破坏进度已从 wall clock 改回 400 game ticks；
- SPR、First Aid New、Pillager’s Gun 均已有 Fabric 26.1.2 发行物，已恢复可选兼容；
...
26.1.2 原生替代、已合并到新 hook 的行为、共享对象/数据驱动 API 的重设计，以及确实没有 Fabric 26.1.2
目标的 LSO/MTS/VS 系列。
...
- [x] First Aid New / Sound Physics Remastered / Pillager’s Gun Fabric 26.1.2 可选兼容
