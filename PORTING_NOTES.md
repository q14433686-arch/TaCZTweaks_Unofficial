...
## 9. 26.1.2 分支移植记录（2026-08-18）

26.1.2 与 26.2 同源（均为未混淆 26.x、Java 25、Fabric API 0.155.2、Loader 0.19.3），
移植 = 复制 26.2 工程 + 改版本号 + 修一处 API 差异：

| 项 | 26.2 | 26.1.2 |
|---|---|---|
| `net.minecraft.world.entity.EntityTypes`（复数） | ✅ | ❌ 不存在 |
| 末影人常量 | `EntityTypes.ENDERMAN` | `EntityType.ENDERMAN` |
| `KeyMapping$Category` / `Identifier` / `GuiGraphicsExtractor` / 粒子 SNBT 语法 | ✅ | ✅（一致） |

其余全部一致：**58 个 TaCZ mixin 目标类的 public/protected 签名 + 10 个 `@Shadow`
private 字段/方法，逐个用 `javap -p` 对比 26.1.2 与 26.2 的 R2 jar，零差异**。

依赖版本变化：`minecraft=26.1.2`、`fabric=0.155.2+26.1.2`、`modmenu=18.0.0-alpha.8`、
YACL=`3.9.6+26.1-fabric`、TACZ jar=`TACZ-Refabricated-26.1.2-1.1.8+fabric.26.1.2.R2.jar`。
