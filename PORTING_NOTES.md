...
## 10. 1.21.11 分支移植记录（2026-08-18）

1.21.11 是**混淆版**（与未混淆的 26.x 完全不同），是三个分支里最难的一个。核心差异：

### 构建配置（照抄 TACZ 1.21.11 分支）
- 插件：`net.fabricmc.fabric-loom-remap`（settings.gradle 里声明 1.17.19）
- `mappings loom.officialMojangMappings()` + `loom.mixin.useLegacyMixinAp = true` + refmap
- **Java 21**（toolchain，不能用 JDK 25 编，会链错 class-file 基线）
- fabric-api 0.141.6+1.21.11（老版本，networking 是 5.1.6）
- **所有 mod 依赖必须用 `mod*` 配置**（modImplementation/modCompileOnly）让 Loom remap；
  YACL/TACZ 本地 jar 用 `modImplementation(files)/modCompileOnly(files)`，否则它们的
  intermediary 签名与 mojang 源码冲突 → 编译期 class_xxx 错误

### 关键认知：Fabric mod jar 分发时 vanilla 类型是 intermediary 名
TACZ 1.21.11 release jar 里 `ResourceLocation=class_2960`、`Vec3=class_243`、`ItemStack=class_1799`
等。因此：
- **remap=true 的 mixin**（vanilla 目标）：源码写 mojang 名，refmap 自动映射到 intermediary ✅
- **remap=false 的 mixin**（com.tacz 目标）：
  - `method` 描述符含 vanilla 类型 → **必须手写 intermediary 名**（如 `<init>(Lnet/minecraft/class_1299;...)V`）
  - `@At` target 含 vanilla 类型 → **加 `remap = true`**（该 @At 单独映射）
  - 纯 com.tacz 目标 → mojang 名，无需处理

### DFU 9.0.19（1.21.11）vs DFU 10（26.2）
API 形态一致：`dispatch` 要 MapCodec、`Codec.unit` 已删、`RecordCodecBuilder.create` 返回 Codec。
→ 26.2 的 `dispatchBy` 桥接与 `MapCodec.unitCodec` **零改动可用**。

### 1.21.11 与 26.2 的 vanilla API 差异（反适配清单）
| 项 | 26.2 | 1.21.11 |
|---|---|---|
| 资源标识符 | `Identifier` | **`Identifier`**（改名在 1.21.11 就发生了） |
| `FriendlyByteBuf` | writeIdentifier/readIdentifier | writeIdentifier/readIdentifier（一致） |
| `EntityTypes`（复数） | ✅ | ❌ → `EntityType.ENDERMAN` |
| `EntityType.is(TagKey)` | ❌（builtInRegistryHolder().is） | ✅ `.is(tag)` |
| `KeyMapping.Category` | ✅ | ✅（一致，Category.register(Identifier)） |
| fabric keybinding | KeyMappingHelper（keymapping v1） | **KeyBindingHelper.registerKeyBinding**（keybinding v1） |
| 权限 | permissions().hasPermission(Permissions.X) | 一致 |
| `Level.gameTime` | ❌ | ✅（BlockBreakingManager 改回 getGameTime） |
| `Level.destroyBlock` | 4 参 | **3 参** |
| `BlockStateParser.parseForBlock` | 直接传 Registry | **需 `BuiltInRegistries.BLOCK as HolderLookup<Block>` 显式转换**（`Registry.asLookup()` 在 1.21.11 不存在；`Registry` 继承 `HolderLookup.RegistryLookup`，cast 是合法 upcast） |
| `SimpleJsonResourceReloadListener` | (Codec, FileToIdConverter) | **(Codec, FileToIdConverter)**（一致！gson 版是 1.20.1） |
| `PayloadTypeRegistry` | serverboundPlay()/clientboundPlay() | **playC2S()/playS2C()**（networking 5.1.6） |
| `Entity.isUnderWater` | Entity 上 | LivingEntity 上（getShooter() 返回 LivingEntity，无需改） |
| 粒子语法 | SNBT | SNBT（一致，1.21.4+） |

### lambda 编号差异（backport 重编译导致）
只有 `GunAnimationStateContext.lambda$shouldSlide$0`（26.2）→ **`$16`**（1.21.11）需要改；
`tickAnimation$0`、`inspect$0`、`initCache$0` 三个编号一致。具名 hook（runShootCycle、
getBlockHitResult、reloadWithDisplay、reloadWithIndex、shootInternal）全部保留。

### 编译期 warning（无害）
remap=false 的 mixin 里 `method="<init>(class_1299...)"` 会报 "Cannot find target method"，
因为编译期 TACZ 是 mojang（modCompileOnly remap 后）、描述符是 intermediary；运行时匹配
玩家装的 intermediary TACZ，正常注入。refmap 已验证：vanilla mixin + @At(remap=true) 的
映射全部正确生成。

### §10.1 R1 → R2 修复（启动崩溃，2026-08-18）
**症状**：1.21.11 实机启动即崩。`Mixin apply for mod tacztweaks failed ...
features.EntityKineticBulletMixin ... @Inject ... tacztweaks$tick$handleSounds could not
find any targets matching 'tick' in com/tacz/guns/entity/EntityKineticBullet`，连带
`Could not execute entrypoint stage 'main' ... provided by 'tacz'`。

**根因**：`EntityKineticBulletMixin` 是 `@Mixin(remap=false)`，它的 `@Inject(method="tick")`
按字面量匹配——但 1.21.11 运行时 `EntityKineticBullet.tick()` 是**从 `Entity` 继承/重写
的 vanilla 方法**，在 intermediary 里叫 `method_5773`，运行时根本没有 `tick` 这个名字。
remap=false 意味着 loom 不会为它生成 refmap 条目，于是注入点找不到目标。

**修复**：`@Inject(method = "tick", ...)` → `method = "method_5773"`（`Entity.tick()` 的
intermediary 名，与 `gun/LocalPlayerMixin` refmap 里 `tick→method_5773` 一致）。已对 TACZ
1.21.11 jar javap 确认：`public void method_5773()` 存在、`onBulletTick()` 为 TACZ 自有名。

**教训**：remap=false 的 mixin 里，凡是「TACZ 类上继承/重写的 vanilla 方法」都必须写
intermediary 名，和「手写 `<init>` 描述符」是同一类坑。R1 交付时只转换了 `<init>` 描述符，
漏掉了 `tick`。

**R2 与 R1 的字节码 diff**：全部 50 个 mixin 的注解逐类比对，仅此一处差异
（`method=["tick"]` → `method=["method_5773"]`）；refmap、fabric.mod.json、
tacztweaks.mixins.json 与 R1 完全一致。

### 附：YACL 加载旧 Forge 配置的报错（非致命）
若 `config/tacztweaks.json` 是**原版 Forge TaCZTweaks** 留下的旧格式
（顶层键 `general/keyActions/gameplay/audioAndVisuals/balancing`），YACL 会对 6 个
分类条目报 `Failed to decode entry gun/modifiers/...: No key ...`。这是**非致命**的：
YACL 会退回默认值并在下次保存时改写为新格式。想彻底消除日志报错就删掉旧配置文件。

### §10.2 R2 → R3 修复（SoundPlayManagerMixin 注入失败，2026-08-18）
**症状**：R2 修好 EntityKineticBullet 后，进到客户端入口 `TaCZFabricClient` 又崩：
`Mixin transformation of com.tacz.guns.client.sound.SoundPlayManager failed`，根因是
`tacztweaks$playReloadSound$broadcast0 ... failed injection check, (0/1) succeeded`。

**根因**：1.21.11 的 TACZ 把 `playReloadSound` / `playInspectSound` **重构**了——不再像
26.x 那样直接调 5 参 `playClientSound(entity,id,vol,pitch,dist)`（两次，ordinal 0/1），
而是走新的私有方法 `playCompositeAnimationContainerSound(entity, id)`，其内部才调
**10 参** `playClientSound(entity,id,1.0,1.0,dist,false,limit,true,false,true)`。
R1/R2 里那 4 个针对 `playReloadSound`/`playInspectSound` 的 `@WrapOperation(PC5)`
在 1.21.11 字节码里扫到 0 个目标 → 注入失败。（R1 根本没跑到这一步，是**潜伏 bug**。）

**修复**：删掉那 4 个 WrapOperation，改成**一个** `@WrapOperation(method =
"playCompositeAnimationContainerSound", at = @At(INVOKE, target = 10 参 playClientSound,
remap = true))`——该私有方法只有 `playReloadSound`/`playInspectSound` 两个调用点
（已 javap 确认），等价覆盖，且拿到的 volume/pitch/distance 是真实值。

**教训**：26.x → 1.21.11 不只是「混淆 + 反适配 API」，TACZ 自身内部实现也被重构过。
所有 com.tacz 目标的 mixin 都必须**对 1.21.11 的 jar 重新逐类 javap**，不能假定
26.x 的调用结构还在。本次对 56 个 com.tacz `@At` 目标做了全量比对，只有这一处真的
不匹配；其余（含 4 个 vanilla 目标）都经 refmap/intermediary 验证无误。
