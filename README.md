...
**TaCZ Tweaks 的 Fabric 26.2 社区移植版**，适配
...
- 完整全局修饰器：伤害、玩家伤害、爆头、穿甲、弹速、重力、摩擦、举枪时间、
  各姿态扩散、射速和后坐力；爆炸弹对玩家伤害也走 `playerDamage`；
...
- `bullet_interactions`：方块/实体/盾牌规则、破坏进度、替换、穿透、伤害；
...
MixinExtras 最低版本。可再传 `--minecraft-jar <loom生成的26.2.jar>` 做原版类方法审计，或传
