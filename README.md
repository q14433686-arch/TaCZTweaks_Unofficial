...
| shield 交互 | 接入 26.1.2 `BlocksAttacks` 组件，支持剩余伤害、耐久和禁用时长 |
| airspace 混响条件音 | 可选兼容 Sound Physics Remastered 1.5.1+26.1.2 Fabric，使用真实 ray-count 缩放 |
| predicate / tier | 使用 26.1.2 实际 `advancements.criterion` predicate 与 `ToolMaterial` 错误挖掘标签 |
| burst/pellet 选择器 | 挂到目标端 R2 的 `spawnProjectiles` / `runShootCycle` 稳定 hook |
| First Aid | 兼容 First Aid New 1.2.8 Fabric 26.1.2：记录命中位置，并让其投射物部位伤害分配识别 TaCZ bullet damage tag |
| Pillager’s Gun | 兼容 3.2.2 Fabric 26.1.2 的袭击者友伤规则，并读取其 `friendlyFire` 配置 |
...
截至 **2026-08-18**，以下项目没有 Fabric 26.1.2 目标，因此不保留无效配置开关：

- Legendary Survival Overhaul：公开新版为 NeoForge，26.1.2 仍是计划/实验状态；
- Valkyrien Skies：公开 Fabric 版仍停在 1.20.1；
- MTS / Immersive Vehicles：未找到 Fabric 26.1.2 发行版。

这表示“当前没有可注入的模组”，不是 Minecraft/Fabric API 无法实现。First Aid、Sound Physics 和
Pillager’s Gun 过去也被误写为不存在，现已根据 26.1.2 实际发行版更正。
...
./gradlew build
# build/libs/tacztweaks-2.14.2+fabric.26.1.2.R2.jar
```
