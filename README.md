...
**中文：TaCZ Tweaks 非官方社区移植。** 本分支 `26.3` 是 **Fabric 26.3** 线，适配
[`TaCZ_Refabricated_Unofficial`](https://github.com/q14433686-arch/TaCZ_Refabricated_Unofficial)。
...
| airspace 混响条件音 | 可选兼容 Sound Physics Remastered Fabric 版，使用真实 ray-count 缩放 |
| predicate / tier | 使用 26.3 包名的 advancement predicate 与 `ToolMaterial` 错误挖掘标签 |
| burst/pellet 选择器 | 挂到目标端 R2 的 `spawnProjectiles` / `runShootCycle` 稳定 hook |
| First Aid | 兼容 First Aid New 1.3.x Fabric：记录命中位置、按投射物分配部位伤害，并覆盖其会导致 pain post chain 重载失败的多余 DynamicTransforms import |
| Pillager’s Gun | 兼容 3.3.5 Fabric 的袭击者友伤规则，并读取其 `friendlyFire` 配置 |
...
截至 **2026-08-18**，以下项目没有对应 Fabric 目标，因此不保留无效配置开关：

- Legendary Survival Overhaul：公开新版为 NeoForge，Fabric 仍是计划/实验状态；
- Valkyrien Skies：公开 Fabric 版仍停在 1.20.1；
- MTS / Immersive Vehicles：未找到对应 Fabric 发行版。
...
| 可选 | Pillager’s Gun (Unofficial Port) | >=3.3.5、<3.4.0 Fabric |
| 运行环境 | Java | >=25 |
