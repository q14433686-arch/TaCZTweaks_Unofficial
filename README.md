...
This branch (`26.3`) is the **Fabric 26.3** line for
[`TaCZ_Refabricated_Unofficial`](https://github.com/q14433686-arch/TaCZ_Refabricated_Unofficial).
The same repository also maintains Fabric/NeoForge lines for 26.3, 26.2, 26.1.2 and 1.21.11 — see
...
**中文：TaCZ Tweaks 非官方社区移植。** 分支 `26.3` 是 **Fabric 26.3** 线，适配
[`TaCZ_Refabricated_Unofficial`](https://github.com/q14433686-arch/TaCZ_Refabricated_Unofficial)。
...
- 动态/静态匍匐俯仰限制和 26.3 `AvatarRenderer` 匍匐过渡平滑；
...
| 匍匐 `visualTweak` | 精确挂到 `AvatarRenderer.setupRotations(AvatarRenderState, …)`，26.3 的 `isVisuallySwimming` 字段已上移至 `HumanoidRenderState`（FIELD owner 同步），不使用脆弱局部变量序号 |
| melee 方块交互 | 枪托/刺刀及目标端内置 LRTactical `IMeleeWeapon#performAttack` 均接入 |
| shield 交互 | 接入 26.3 `BlocksAttacks` 组件，支持剩余伤害、耐久和禁用时长 |
| airspace 混响条件音 | 可选兼容 Sound Physics Remastered 1.5.1+26.3 Fabric，使用真实 ray-count 缩放 |
| predicate / tier | 使用 26.3 新包名的 advancement predicate 与 `ToolMaterial` 错误挖掘标签 |
| burst/pellet 选择器 | 挂到目标端 R1 的 `spawnProjectiles` / `runShootCycle` 稳定 hook |
| First Aid | 兼容 First Aid New 1.3.x Fabric 26.2：记录命中位置、按投射物分配部位伤害，并覆盖其会导致 pain post chain 重载失败的多余 DynamicTransforms import（截至 2026-09-22 尚无 26.3 构建，接口代码随其出现时启用） |
| Pillager’s Gun | 兼容 3.3.5 Fabric 26.2 的袭击者友伤规则，并读取其 `friendlyFire` 配置（截至 2026-09-22 尚无 26.3 构建，接口代码随其出现时启用） |

`thirdPersonGunRenderingFix` 没有重复实现：TaCZ Refabricated R1 已在
...
截至 **2026-09-22**，以下项目没有 Fabric 26.3 目标，因此不保留无效配置开关：

- Legendary Survival Overhaul：公开新版为 NeoForge，26.3 仍是计划/实验状态；
- Valkyrien Skies：公开 Fabric 版仍停在 1.20.1；
- MTS / Immersive Vehicles：未找到 Fabric 26.3 发行版。

这表示“当前没有可注入的模组”，不是 Minecraft/Fabric API 无法实现。First Aid、Sound Physics 和
Pillager’s Gun 过去也被误写为不存在，现已根据实际发行版更正。
...
|---|---|---|
| 必需 | Minecraft | 26.3 |
| 必需 | Fabric Loader | >=0.19.5、<0.20.0 |
| 必需 | Fabric API | >=0.160.7+26.3、<0.162.0 |
| 必需 | [UNOFFICIAL] TaCZ Refabricated | **1.1.8+fabric.26.3.R1 及之后的 R<n> 版本**（运行时按当前 26.3 发布系列校验） |
| 必需 | Fabric Language Kotlin | >=1.14.1、<1.15.0 |
| 必需 | YetAnotherConfigLib | 3.9.7+26.3-fabric |
| 可选 | Sound Physics Remastered | 1.5.1+26.3 Fabric |
| 可选 | First Aid New | >=1.3.0、<1.4.0 Fabric（shader 覆盖仅按 1.3.x 验证；截至 2026-09-22 该项目尚无 26.3 构建） |
| 可选 | Pillager’s Gun (Unofficial Port) | >=3.3.5、<3.4.0 Fabric（截至 2026-09-22 该项目尚无 26.3 构建） |
| 运行环境 | Java | >=25 |
...
./gradlew build
# build/libs/tacztweaks-2.14.2+fabric.26.3.Beta-1.jar
```

`scripts/audit_port.py` 会检查 mixin 注册/目标方法、无行为配置项、语言键一致性、
MixinExtras 最低版本，以及模组图标的元数据路径、尺寸、SHA-256 与许可声明。`./gradlew build`
也会通过 `checkModIcon` 执行图标回归门禁。可再传 `--minecraft-jar <loom生成的26.3.jar>` 做原版
