...
Data packs do not restore removed legacy config options. On this 1.21.11 branch, remove
`thirdPersonGunRenderingFix` from old configs; the fix ships inside TaCZ Refabricated Unofficial
`1.1.8+fabric.1.21.11.R2`. The legacy compatibility switches (`lsoCompat`, `vsCollisionCompat`,
`vsExplosionCompat`, `mtsFix`) still exist on this branch and are no-ops without their mods.
