...
def audit_firstaid_shader_overrides() -> list[str]:
    ...
    for name, required_block in expected.items():
        ...
        if not path.is_file():
            errors.append(f"missing First Aid 26.3 shader compatibility override: {name}")
            continue
        text = path.read_text(encoding="utf-8")
...
def main() -> int:
    ...
    # libs/*.jar are not committed (see .gitignore) and are rebuilt from
    # RESOURCE_IMPORT_MANIFEST.tsv. Without the TaCZ jar every mixin target looks missing,
    # which would bury the real findings under a hundred bogus errors. Say so once instead.
    if not TA_CZ_JAR.is_file():
        print(
            "AUDIT: cannot run - the TaCZ target jar is missing:\n"
            f"  {TA_CZ_JAR.relative_to(ROOT)}\n"
            "Run `python3 scripts/download_dependencies.py` first "
            "(CI does this before every audit).",
            file=sys.stderr,
        )
        return 2
