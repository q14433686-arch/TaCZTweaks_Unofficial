...
HEADER = (
    "path\tsource_url\tupstream_project\tupstream_version\tsha256\tlicense\tuse\t"
    "bundled_in_release_jar\tretrieved_or_verified_utc\tnotes\n"
)
...
def _workspace(tmp_path: Path, sha_cell: str) -> tuple[Path, str]:
    ...
    payload = b"not-really-a-jar-but-bytes-are-bytes"
    ...
    (tmp_path / "RESOURCE_IMPORT_MANIFEST.tsv").write_text(
        HEADER
        + f"libs/dep.jar\tfile://{source}\tproj\t1.0\t{cell}\tMIT\tuse\tno\t2026-09-22\tnote\n",
        encoding="utf-8",
    )
