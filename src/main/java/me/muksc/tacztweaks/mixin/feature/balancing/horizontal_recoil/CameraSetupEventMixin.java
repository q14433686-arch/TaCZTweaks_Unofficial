    ...
    private static PolynomialSplineFunction tacztweaks$initialCameraRecoil$horizontalRecoilModifier(
        ...
        try {
            ext.tacztweaks$setDynamicModifierMapper(value -> {
                ...
                double recoil = Config.Balancing.HorizontalRecoil.eval(Math.abs(value));
                if (state.crawling()) recoil = Config.Balancing.CrawlHorizontalRecoil.eval(recoil);
                double aimingModifier = Config.Balancing.AimHorizontalRecoil.eval(recoil) - recoil;
