 ...
 *
 * 26.2 notes:
 * - `shield` is implemented via mixin {@code hurtServer} (no Forge {@code ShieldBlockEvent});
 *   the item matcher is id/tag instead of the deleted {@code ItemPredicate};
 * - `block_break` has no {@code tier} field (use hardness / block tags instead).
 */
sealed class BulletInteraction(
    ...
    class Block(
        ...
        sealed class BlockBreak(
            ...
            object Never : BlockBreak(EBlockBreakType.NEVER, Blocks.AIR.defaultBlockState().blockInput(), ValueRange.DEFAULT, false) {
                val CODEC: Codec<Never> = MapCodec.unitCodec(Never)
            }
            ...
            class Instant(
                ...
                hardness: ValueRange,
                drop: Boolean
            ) : BlockBreak(EBlockBreakType.INSTANT, replaceWith, hardness, drop) {
                companion object {
            ...
            class Count(
                ...
                hardness: ValueRange,
                drop: Boolean
            ) : BlockBreak(EBlockBreakType.COUNT, replaceWith, hardness, drop) {
                companion object {
            ...
            class FixedDamage(
                ...
                hardness: ValueRange,
                drop: Boolean
            ) : BlockBreak(EBlockBreakType.FIXED_DAMAGE, replaceWith, hardness, drop) {
                companion object {
            ...
            class DynamicDamage(
                ...
                hardness: ValueRange,
                drop: Boolean
            ) : BlockBreak(EBlockBreakType.DYNAMIC_DAMAGE, replaceWith, hardness, drop) {
                companion object {
    ...
    class Shield(
        ...
        class ItemMatch(
            val items: List<net.minecraft.resources.Identifier>,
            val tags: List<net.minecraft.resources.Identifier>
        ) {
            fun matches(stack: net.minecraft.world.item.ItemStack): Boolean {
                ...
                if (id != null && id in items) return true
                return tags.any { tagId ->
                    stack.`is`(net.minecraft.tags.TagKey.create(net.minecraft.core.registries.Registries.ITEM, tagId))
                }
            }

            companion object {
                val ANY = ItemMatch(emptyList(), emptyList())
                val CODEC: Codec<ItemMatch> = RecordCodecBuilder.create { it.group(
                    Codec.list(net.minecraft.resources.Identifier.CODEC).strictOptionalFieldOf("items", emptyList()).forGetter(ItemMatch::items),
                    Codec.list(net.minecraft.resources.Identifier.CODEC).strictOptionalFieldOf("tags", emptyList()).forGetter(ItemMatch::tags)
                ).apply(it, ::ItemMatch) }
            }
        }
        ...
        companion object {
            ...
            val CODEC: Codec<Shield> = RecordCodecBuilder.create { it.group(
                ...
                ItemMatch.CODEC.strictOptionalFieldOf("items", DEFAULT.items).forGetter(Shield::items),
